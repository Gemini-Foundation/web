    ...
    <a href={`/${entry.collection}/${entry.slug}`} class="group p-4 gap-3 flex items-center border rounded-lg transition-colors duration-300 ease-in-out
      ...
      <div class="w-full blend
        ...
        <div class="flex flex-wrap items-center gap-2">
          ...
          <div class="text-sm uppercase text-light-secondary font-medium">
        ...
        <div class="text-sm line-clamp-2 text-light-text-secondary">
          {entry.data.summary}
        </div>
        <ul class="flex flex-wrap mt-2 gap-1">
          {entry.data.tags.map((tag: string) => (
            <li class="text-xs uppercase py-0.5 px-2 rounded
              bg-light-bg-accent text-light-text-secondary">
              {truncateText(tag, 20)}
            </li>
          ))}
        </ul>
